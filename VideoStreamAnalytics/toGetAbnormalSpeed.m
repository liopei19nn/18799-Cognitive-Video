...
speed_detection_limit_upper = 40;
speed_detection_limit_lower = 30;
...
%     t = t+1;
    ...
    bbox = step(blobAnalysis, boolean(v));
     ...
     for j = 1:size(bbox,1)
        ...
        if width*height > 0
            if (x+ width < col) && (y + height  < row)
                ...
                if m < speed_detection_limit_upper && m > speed_detection_limit_lower    %%set threshold for detecting cars
     ...
     for p = 1:size(velocity_mark,1)
         if velocity_mark(p,3) < speed_detection_limit_upper && velocity_mark(p,3) > speed_detection_limit_lower
